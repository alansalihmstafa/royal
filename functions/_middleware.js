...
async function otpHash(code, otpId, env) {
  const secret = env.ADMIN_SESSION_SECRET || "change-this-session-secret";
  return sha256(`${code}:${otpId}:${secret}`);
}
...
async function verifyTurnstile(request, token, env) {
  if (!env.TURNSTILE_SECRET_KEY) {
    return { success: false, message: "Turnstile secret key missing." };
  }
...
async function sendCodeEmail(email, code, env) {
  ...
  if (!response.ok) {
    return {
      success: false,
      message: data.message || "Email failed to send."
    };
  }
...
function css() {
  return `
    ...
    button {
      ...
      border: 0;
      border-radius: 12px;
      background: linear-gradient(120deg,#7c5cff,#6741e0);
      color: white;
      font-weight: 800;
      cursor: pointer;
      font-size: 15px;
...
function loginPage(env, error = "") {
  ...
  const turnstileHtml = siteKey
    ...
    : `<div class="err">TURNSTILE_SITE_KEY missing.</div>`;
...
<body>
  <form class="box" method="POST" action="/royal-owner-panel">
    ...
    <p>
      Enter owner email and password. If correct, a code will be sent to
      <b>${allowedEmail}</b>.
    </p>

    <input name="email" type="email" placeholder="Owner email" autocomplete="email" required>
    <input name="password" type="password" placeholder="Owner password" autocomplete="current-password" required>
...
<body>
  <form class="box" method="POST" action="/royal-owner-panel">
    ...
    <input name="code" type="text" inputmode="numeric" maxlength="6" placeholder="6-digit code" autocomplete="one-time-code" required>

    <button type="submit">Verify and open admin</button>

    <a class="ghost" href="/royal-owner-panel">Start again</a>
...
export async function onRequest(context) {
  ...
  if (!env.ROYAL_KV) {
    return html("Missing ROYAL_KV binding.", 500);
  }

  const expectedSession = await sessionToken(env);
  const currentSession = getCookie(request, "royal_owner_session");

  if (request.method === "GET" && currentSession === expectedSession) {
    return next();
  }
  ...
  if (action === "send-code") {
    ...
    if (!limited.allowed) {
      return html(
        loginPage(env, "Too many attempts. Wait 10 minutes and try again."),
        429
      );
    }
    ...
    if (!ownerEmail) {
      return html(
        loginPage(env, "ADMIN_EMAIL missing in Cloudflare variables."),
        500
      );
    }

    if (email !== ownerEmail || providedPassword !== realPassword) {
      return html(loginPage(env, "Wrong email or password."), 401);
    }
    ...
    const record = {
      email,
      hash: await otpHash(code, otpId, env),
      attempts: 0,
      createdAt: Date.now()
    };

    await env.ROYAL_KV.put(`otp:${otpId}`, JSON.stringify(record), {
    ...
    if (!sent.success) {
      return html(loginPage(env, sent.message), 500);
    }

    return html(
      codePage("Code sent. Check your email. It expires in 10 minutes."),
      200,
      {
  ...
  if (action === "verify-code") {
    ...
    if (!otpId) {
      return html(loginPage(env, "Code session expired. Start again."), 401);
    }
    ...
    if (!raw) {
      return html(loginPage(env, "Code expired. Start again."), 401);
    }
    ...
    if (Number(record.attempts || 0) >= MAX_OTP_ATTEMPTS) {
      ...
      return html(loginPage(env, "Too many wrong codes. Start again."), 401);
    }

    const providedCode = String(form.get("code") || "").trim();
    const providedHash = await otpHash(providedCode, otpId, env);

    if (providedHash !== record.hash) {
      record.attempts = Number(record.attempts || 0) + 1;

      await env.ROYAL_KV.put(key, JSON.stringify(record), {
        expirationTtl: OTP_TTL_SECONDS
      });
    ...
    return new Response(null, {
      ...
      headers: {
        ...
        "Set-Cookie": `royal_owner_session=${encodeURIComponent(expectedSession)}; Path=/; HttpOnly; Secure; SameSite=Strict; Max-Age=86400`
      }
    });
  }
